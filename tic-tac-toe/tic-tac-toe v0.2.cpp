	...
	while (true) {
		...
		*/
		...
		if (CheckWin(Player0)) { // if 'true' in terms of Player0 winning:
			RefreshBoard(); // clear the screen and display the board with new values
			cout << "\t\t      Player " << Player0 << " Wins!\n"; // Tell both players that player0 is the winner
			break; // stop the game
		} else if (CheckWin(Player1)) { // if 'true' in terms of player1 winning:
			RefreshBoard(); // do same
